...
class SecurityAnalyzer:
    ...
    SECRET_PATTERNS: ClassVar[list[tuple[re.Pattern[str], str]]] = [
        (
            re.compile(
                r"(?i)(api[_-]?key|apikey|secret|password|token|auth[_-]?key)"
                r"\s*[:=]\s*[\"']?([a-zA-Z0-9_\-]{20,})[\"']?"
            ),
            "hardcoded_secret",
        ),
        ...
        (
            re.compile(r"(?i)(github[_-]?token|gh[_-]?token|ghp_[a-zA-Z0-9]{36})"),
            "github_token",
        ),
        (
            re.compile(r"(?i)(slack[_-]?token|xoxb-[a-zA-Z0-9\-]+)"),
            "slack_token",
        ),
        (
            re.compile(r"(?i)(npm[_-]?token|npm_[a-zA-Z0-9]{36})"),
            "npm_token",
        ),
        (
            re.compile(r"(?i)(docker[_-]?password|docker[_-]?token)"),
            "docker_credentials",
        ),
    ]

    # Patterns for command injection vulnerabilities
    INJECTION_PATTERNS: ClassVar[list[tuple[re.Pattern[str], str]]] = [
        (
            re.compile(
                r"\$\{\{\s*(github\.event\.(issue|pull_request|comment|release|repository)"
                r"\.[^}]+)\s*\}\}"
            ),
            "github_event_injection",
        ),
        (
            re.compile(r"\$\{\{\s*(github\.event\.[^}]+)\s*\}\}"),
            "github_context_injection",
        ),
        (
    ...
    def _check_hardcoded_secrets(self, content: str, path: str) -> None:
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern, category in self.SECRET_PATTERNS:
                ...
                if match:
                    severity = (
                        "critical" if category in ("aws_credentials", "github_token") else "high"
                    )
                    self.findings.append(
                        SecurityFinding(
                            ...
                            description=(
                                f"Possible secret detected in workflow file: {match.group()[:50]}"
                            ),
                            file_path=path,
        ...
        if workflow.permissions is None:
            self.findings.append(
                SecurityFinding(
                    ...
                    description=(
                        "Workflow does not define explicit permissions. "
                        "Defaults to read-all for contents."
                    ),
                    file_path=path,
            ...
            for key, value in workflow.permissions.items():
                if value in ("write-all", "admin") or (
                    isinstance(value, str) and "write" in value and key == "contents"
                ):
                    self.findings.append(
                        SecurityFinding(
                            severity="high",
                            ...
                            file_path=path,
                            remediation=(
                                f"Restrict {key} permission to minimal "
                                "required scope (e.g., 'read')."
                            ),
                            cwe_id="CWE-250",
                        )
                    )
                ...
                if "@" not in uses:
                    self.findings.append(
                        SecurityFinding(
                            ...
                            description=(
                                f"Action '{uses}' is not pinned to a specific "
                    ...
                    if not re.match(r"^[a-f0-9]{40}$", version):
                        self.findings.append(
                            SecurityFinding(
                                ...
                                description=(
                                    f"Action '{uses}' is pinned to tag '{version}' "
                                    "which can be moved."
                                ),
                                file_path=path,
                                remediation=(
                                    f"Pin to commit SHA instead of tag: {uses.split('@')[0]}@<sha>"
                                ),
                                cwe_id="CWE-829",
                            )
                        )
    ...
    def _check_command_injection(self, content: str, path: str) -> None:
        ...
        lines = content.split("\n")
        for line_num, line in enumerate(lines, 1):
            for pattern, _category in self.INJECTION_PATTERNS:
                match = pattern.search(line)
                if match:
                    self.findings.append(
                        SecurityFinding(
                            ...
                            description=(
                                "Untrusted GitHub context interpolated into shell command: "
                                f"{match.group()[:100]}"
                            ),
                            file_path=path,
                            line_number=line_num,
                            remediation=(
                                "Avoid interpolating untrusted input into shell commands. "
                                "Use action inputs instead of shell interpolation."
                            ),
                            cwe_id="CWE-78",
                        )
                    )
                    ...
                    if owner not in trusted_owners:
                        self.findings.append(
                            SecurityFinding(
                                ...
                                file_path=path,
                                remediation=(
                                    "Verify the action source, pin to commit SHA, "
                                    "and consider using trusted alternatives."
                                ),
                                cwe_id="CWE-829",
                            )
                        )
                    ...
                    if value in ("write-all", "admin") or (
                        ...
                        self.findings.append(
                            SecurityFinding(
                                severity="medium",
                                category="permissions",
                                title=(f"Job has overly permissive {key} permission"),
                                description=f"Job grants '{value}' permission for {key}.",
                                file_path=path,
                                remediation=(
                                    f"Restrict {key} permission to minimal required scope."
                                ),
                                cwe_id="CWE-250",
                            )
                        )
